...
## Corpus Check
- 137 files · ~47,394 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 833 nodes · 1664 edges · 67 communities (51 shown, 16 thin omitted)
...
Cohesion: 0.12
Nodes (29): Container(), ContainerProps, ProtectedRoute(), ProtectedRouteProps, StatusBadge(), StatusBadgeProps, RootLayout(), rootElement (+21 more)
...
Cohesion: 0.09
Nodes (35): create_patient(), get_patient(), lookup_patient_by_mobile(), get, patch, post, Session, UUID (+27 more)
...
### Community 40 - "User"
Cohesion: 0.06
Nodes (61): Protected Admin demonstration routes., Base, Declarative base and shared metadata. A consistent naming convention makes…, Reusable model mixins (infrastructure only — no tables defined here). These…, Adds a UUID ``id`` primary key (never a mobile number / RFID / biometric)., Adds UTC ``created_at`` / ``updated_at`` columns., TimestampMixin, UUIDPrimaryKeyMixin (+53 more)

### Community 41 - "ConfigService"
Cohesion: 0.08
Nodes (32): list_department_consultants(), list_departments(), get, Session, UUID, Department configuration endpoints., list_languages(), get (+24 more)

### Community 42 - "SessionService"
Cohesion: 0.14
Nodes (30): clear_session(), complete_session(), create_session(), get_consents(), get_session(), get, patch, post (+22 more)
...
- **Why does `utcnow()` connect `User` to `test_auth.py`, `system_service.py`, `endpoints/identity.py`, `patients.py`, `SessionService`, `AuthService`, `auth_service.py`?**
